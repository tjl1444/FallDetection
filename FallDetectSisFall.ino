...
// Place board in same orientation as SisFall Dataset X to the left, Y downwards
...
float vector_mag[10];
Imu_position tilt[128];

uint16_t buffer_index = 0;
uint8_t accel_index = 0;
uint16_t accel_trigger = NULL;
uint16_t tilt_trigger = NULL;
...
//  double x_rot = atan(Y_out / sqrt(pow(X_out, 2) + pow(Z_out, 2))) * 180 / PI;
//  double y_rot = atan(-1 * X_out / sqrt(pow(Y_out, 2) + pow(Z_out, 2))) * 180 / PI;

  double x_rot = atan(-1 * Z_out / sqrt(pow(X_out, 2) + pow(Y_out, 2))) * 180 / PI;
  double z_rot = atan(-1 * X_out / sqrt(pow(Y_out, 2) + pow(Z_out, 2))) * 180 / PI; 

  // Low Pass Filter
  x_rotF = 0.90 * x_rotF + 0.10 * x_rot;
  z_rotF = 0.90 * z_rotF + 0.10 * z_rot;

  double accelerationMagnitude = sqrt((X_out * X_out) + (Y_out * Y_out) + (Z_out * Z_out));
 
//  Serial.print(roll); Serial.print(" ");
//  Serial.print(pitch); Serial.print(" ");
//  Serial.print(buffer_index); Serial.print(" ");
//  Serial.println(accelerationMagnitude);

  Serial.print(x_rotF); Serial.print(" ");
  Serial.print(z_rotF); Serial.print(" ");
  Serial.println(accelerationMagnitude);
